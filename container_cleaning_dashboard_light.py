...
def render_queue_item(item, score_data):
    ...
    # HTML ditulis dalam satu baris untuk menghindari bug Code Block di Streamlit Markdown
...
def main():
    ...
    with col1:
        ...
        with st.form("input_form", border=False):
            ...
            st.markdown(f"""
            <div style="background-color: #f8f9fa; border-radius: 10px; padding: 12px; margin: 15px 0; border: 1px solid #e0e0e0;">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <span style="color: #666; font-size: 13px;">Estimasi Durasi</span>
                    <span style="color: #2e7d32; font-size: 16px; font-weight: bold;">{score_data['estimated_duration']} mnt</span>
                </div>
                <div class="divider" style="margin: 8px 0;"></div>
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <span style="color: #666; font-size: 13px;">Priority Score</span>
                    <div>{get_priority_badge_html(priority_class)}</div>
                </div>
                <div style="display: flex; justify-content: space-between; margin-top: 8px;">
                    <span style="color: #888; font-size: 11px;">Base: {score_data['base_priority']} x Cargo: {score_data['cargo_multiplier']} x Dirt: {score_data['dirt_multiplier']}</span>
                    <span style="color: #666; font-size: 11px;">CR: {score_data['critical_ratio']}x</span>
                </div>
    ...
    with col2:
        ...
        if st.session_state.in_progress:
            ...
            st.markdown(f"""
            <div style="background-color: #ffffff; border-radius: 15px; padding: 20px; margin-bottom: 15px; border: 1px solid #e0e0e0; box-shadow: 0 2px 8px rgba(0,0,0,0.06);">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                    <span style="font-weight: bold; font-size: 18px; color: #1a1a1a;">{job['id']}</span>
                    <span class="badge-high">Urgensi tinggi</span>
                </div>
                <div style="margin-bottom: 12px;">
                    <span class="tag tag-type">{job['container_type']}</span>
                    <span class="tag">{job['cargo_type']}</span>
                    <span class="tag">{job['dirt_level']}</span>
                </div>
                <div style="margin-bottom: 10px;">
                    <span class="tag tag-food">Food grade</span>
                    <span class="tag tag-time">Est. {job['estimated_duration']} mnt</span>
                    {ro_html}
                </div>
            </div>
            
            <div class="timer-display">
                <div class="timer-value">{time_display}</div>
                <div class="timer-label">dari estimasi {job['estimated_duration']} menit</div>
            </div>
            
            <div style="margin-top: 15px;">
                <div style="display: flex; justify-content: space-between; margin-bottom: 5px;">
                    <span style="font-size: 12px; color: #aaa;">Progress</span>
                    <span style="font-size: 12px; color: #4caf50;">{int(progress_pct)}%</span>
                </div>
                <div class="progress-bar">
                    <div class="progress-fill" style="width: {progress_pct}%; background: linear-gradient(90deg, #4caf50, #66bb6a);"></div>
                </div>
            </div>
            """, unsafe_allow_html=True)
            ...
            # Ditambahkan unsafe_allow_html agar tidak bocor render box-nya
            st.markdown("""
            ...
            if st.session_state.queue:
                ...
                st.markdown(f"""
                <div style="background-color: #e8f5e9; border: 1px solid #4caf50; border-radius: 10px; padding: 12px; margin: 15px 0;">
                    <div style="font-size: 13px; color: #2e7d32; margin-bottom: 5px;">Next Priority:</div>
                    <div style="font-weight: bold; color: #1a1a1a;">{next_item['id']} &mdash; {next_item['container_type']}</div>
                    <div style="font-size: 12px; color: #666;">{next_item['cargo_type']} | Score: {next_score['final_score']} | ~{next_score['estimated_duration']} mnt</div>
                </div>
                """, unsafe_allow_html=True)
    ...
    with col3:
        ...
        if st.session_state.in_progress:
            ...
            # Pakai st.markdown f-string yang bersih
            active_html = f"""
            <div style="background: linear-gradient(135deg, #1b5e20, #2e7d32); border-radius: 12px; padding: 14px; margin-bottom: 15px; color: white; box-shadow: 0 2px 8px rgba(27,94,32,0.3);">
                <div style="font-size: 11px; opacity: 0.9; margin-bottom: 5px;">SEDANG DIKERJAKAN</div>
                <div style="font-weight: bold; font-size: 15px;">{job['id']}</div>
                <div style="font-size: 12px; opacity: 0.95;">{job['container_type']} &bull; {job['cargo_type']}</div>
        ...
        if st.session_state.queue:
            ...
            for item, score in scored_queue:
                ...
                with col_del:
                    if st.button("🗑️", key=f"del_{item['id']}", help="Hapus antrian"):
